                        ...
                        {% for type in types %}
                            <div class="btn-wrapper col-sm-6 col-md-4 col-lg-3" >
                                {% for log in changelog_current %} <!-- if statement on next line could've been replaced by where filter: | where: "type", type.id -->
                                    ...
                                    {% endif %}
                                {% endfor %}
                                <a class="btn-decktype" href="/tier-list/deck-types/{{type.id}}">
                                ...
                                {% assign dtPage = site.posts | where: "category", "page" | where: "sub-category", "deck-type" | where: "deck-type", type.id | first %}
                                {% if dtPage == null %}
                                    <span class="btn-sub-text">No information yet</span>
                                {% elsif dtPage.hide != null and dtPage.hide %}
                                    <span class="btn-sub-text">No information yet</span>
                                {% else %}
                                    <span class="btn-sub-text">Last updated: {{ dtPage.date | timeago }}</span>
                                {% endif %}
                            </div>
                        {% endfor %}
                    </div>
