        ...
        for month_key in year.keys
          ...
          for decktype_key in month.keys
            ...
            for deck_key in decktype.keys
              ...
              deck_file.puts("---")
              deck_file.puts("")
              deck_file.puts("<h2>{{site.data.decklists.#{year_key}.#{month_key}.#{decktype_key}.#{deck_key}.name}}</h2>")
              deck_file.puts("<h4>{{site.data.decklists.#{year_key}.#{month_key}.#{decktype_key}.#{deck_key}.author}}</h4>")
              deck_file.puts("")
              deck_file.puts("<p style=\"margin-top: 2rem;\">Added {{site.data.decklists.#{year_key}.#{month_key}.#{decktype_key}.#{deck_key}.created}}</p>")
              deck_file.puts("")
              deck_file.puts("<ul class=\"list-group\">")
              deck_file.puts("    <h4 style=\"margin: 1rem 0;\">Main Deck</h4>")
              deck_file.puts("    {% assign main = site.data.decklists.#{year_key}.#{month_key}.#{decktype_key}.#{deck_key}.main %}")
              deck_file.puts("    {% for item in main %}")
              deck_file.puts("        {% include deckbuild.html card=item %}")
              deck_file.puts("    {% endfor %}")
              deck_file.puts("    {% assign extra = site.data.decklists.#{year_key}.#{month_key}.#{decktype_key}.#{deck_key}.extra %}")
